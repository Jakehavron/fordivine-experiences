(function () {
  ...
  function buildVideoPlaceholder(container) {
    ...
    // Reuse the server-rendered priority poster so the browser can discover it immediately.
    var poster = container.querySelector('.fd-deferred-video-poster') || document.createElement('img');
    poster.className = 'fd-deferred-video-poster';
    poster.src = container.dataset.poster;
    poster.alt = '';
    poster.decoding = 'async';
    var priorityPoster = container.dataset.posterPriority === 'high' || container.dataset.ratio === 'landscape';
    poster.loading = priorityPoster ? 'eager' : 'lazy';
    if (priorityPoster) poster.fetchPriority = 'high';
    ...
    if (!poster.parentNode) fragment.appendChild(poster);
