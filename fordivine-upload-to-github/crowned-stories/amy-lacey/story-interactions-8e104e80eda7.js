...
(() => {
  ...
  rows.forEach(row => {
    setOpen(row, motion.matches || Boolean(targetAfterResults));
  ...
  const settleAnchor = () => {
    ...
    if (follows) rows.forEach(row => setOpen(row, true));
...
(() => {
 ...
 const reduced=matchMedia('(prefers-reduced-motion: reduce)');let busy=false,near=false;
 const preparePages=()=>{if(!near)return;const current=book.getCurrentPageIndex();slides.forEach((slide,index)=>{if(Math.abs(index-current)<=1)slide.querySelector('img').loading='eager';});};
 ...
 const render=()=>{preparePages();const current=book.getCurrentPageIndex();prev.disabled=busy||current===0;next.disabled=busy||current===slides.length-1;status.textContent=`Page ${current+1} of ${slides.length}`;};
 ...
 const approach=new IntersectionObserver(entries=>{if(entries.some(entry=>entry.isIntersecting)){near=true;preparePages();approach.disconnect();}},{rootMargin:'1000px'});approach.observe(root);
