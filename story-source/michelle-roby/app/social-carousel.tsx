...
export default function SocialCarousel() {
  ...
  return <section className="chapter social-carousel" id="social-content" aria-labelledby="social-title" aria-roledescription="carousel">
    ...
    <ul id="social-posts" ref={track} className="social-track" aria-label="Michelle’s Instagram posts" tabIndex={0} onKeyDown={event=>{if(event.key==='ArrowRight'||event.key==='ArrowLeft'){event.preventDefault();move(event.key==='ArrowRight'?1:-1);}if(event.key==='Home'||event.key==='End'){event.preventDefault();track.current?.scrollTo({left:event.key==='Home'?0:track.current.scrollWidth,behavior:'auto'});}}}>
      {posts.map((id,index)=><li key={id} className="social-card"><a href={`https://www.instagram.com/p/${id}/`} target="_blank" rel="noopener noreferrer" aria-label={`View Michelle’s Instagram post ${index+1} of ${posts.length} (opens in a new tab)`}><img src={`/crowned-stories/michelle-roby/images/michelle-post-${String(index+1).padStart(2,'0')}.jpg`} alt={`Cover of Michelle Roby’s Instagram post ${index+1}`} width="1080" height="1350" loading="lazy"/><span className="social-card-label">View on Instagram <span aria-hidden="true">↗</span></span></a></li>)}
    </ul>
