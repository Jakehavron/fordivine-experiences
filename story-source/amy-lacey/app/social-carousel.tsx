...
<ul id="social-posts" className="social-track reel-track" aria-label="Six videos created for Amy, alternating creative films and speaking reels" tabIndex={0}>{reels.map(reel=><li key={reel.number} className="social-card reel-card"><div className="reel-player"><video controls playsInline preload="none" poster={`/crowned-stories/amy-lacey/images/reels/amy-reel-${String(reel.number).padStart(2,'0')}.webp`} aria-label={reel.title} width="720" height="1280"><source src={`/crowned-stories/amy-lacey/videos/amy-reel-${String(reel.number).padStart(2,'0')}.mp4`} type="video/mp4"/>Your browser does not support embedded video.</video></div><div className="reel-caption"><span>{String(reel.number).padStart(2,'0')} / {reel.type}</span><h3>{reel.title}</h3></div></li>)}</ul>
